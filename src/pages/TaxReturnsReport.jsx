...
export default function TaxReturnsReport() {
  ...
  // Request واحد فقط -> ShowReTaxReport مرة واحدة فقط
  // =========================================================
  const loadReport =
    useCallback(async () => {
      ...
      try {
        ...
        const response =
          await fetch(
            `${API_BASE_URL}/api/tax-returns-report?${params.toString()}`,
            {
              cache: "no-store",
              headers: {
                Accept: "application/json"
        ...
        if (!response.ok) {
          throw new Error(
            result?.message ||
            result?.error ||
            "تعذر تحميل تقرير مرتجعات الضرائب"
          );
        }
  ...
  const exportCsv = () => {
    ...
    a.href = url;
    a.download =
      `تقرير-مرتجعات-الضرائب-${fromDate}-${toDate}.csv`;
    ...
    return (
      ...
      >
        <Alert severity="error">
          ليس لديك صلاحية تقرير مرتجعات الضرائب.
        </Alert>
      </Box>
    );
  }
