...
const root=process.cwd(),out=path.join(root,'ui-review/hr/preview-build');
webpack({mode:'development',entry:path.join(root,'ui-review/hr/preview.jsx'),output:{path:out,filename:'preview.js'},devtool:false,
  ...
  plugins:[new webpack.DefinePlugin({'process.env':JSON.stringify({NODE_ENV:'development',REACT_APP_API_BASE_URL:'http://127.0.0.1:3102',REACT_APP_API_URL:'http://127.0.0.1:3102'})})],optimization:{minimize:false}
},(err,stats)=>{
