...
(async()=>{
  ...
  const results=JSON.parse(fs.readFileSync('ui-review/hr/dialog-results.json','utf8')).filter(r=>r.dialog);
  ...
  for(const [page,label] of cases)for(const width of [1366,390]){
    if(results.some(r=>r.page===page&&r.label===label&&r.viewport===width))continue;
    await send('Emulation.setDeviceMetricsOverride',{width,height:844,deviceScaleFactor:1,mobile:false});
    await send('Page.navigate',{url:'http://127.0.0.1:3102/?page='+page});for(let i=0;i<80;i++){await wait(100);if(await evaluate('!!document.querySelector("#root button")'))break;}await wait(800);
